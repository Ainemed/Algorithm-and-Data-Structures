...
/**
 * @brief Performs Counting Sort on a vector of integers based on the i-th digit (in base k).
 * 
 * This version of Counting Sort is used internally by Radix Sort to sort elements
 * by a specific digit, from least to most significant digit.
 * 
 ...
 */
template<typename T>
void countingSortAsc(vector<T>& arr, int k, int i)
{
...
/**
 * @brief Sorts a vector of integers using the Radix Sort algorithm in ascending order.
 * 
 * Radix Sort processes the digits of each number starting from the least significant digit
 * to the most significant, using Counting Sort as a stable sorting subroutine.
 * 
 ...
 */
template<typename T>
void radixSortAsc(vector<T>& arr, int d, int k)
{
    for (int i = 1; i <= d; i++) {  // Start from least significant digit
        countingSortAsc(arr, k, i);
    }
}
